...
# I.2  Meta regression: Figure ------
  ...
  title(ylab = "Absolute Risk Difference (%)",
        xlab = "Cardiovascular Mortality Rate (/100py) in Control Group", 
...
# I.3  Meta regression: Figure for secondary outcomes -----
  ...
  par(oma = c(2,1,0,0), mfrow = c(1,3), mar = c(5,5,3,2)*0.8)
    ...
    # Axes titles
      ...
      title(xlab = "Cardiovascular Mortality Rate (/100py) in Control Group", cex.lab = 1, line = 2.5)
    # Axes
    axis(1, 
         at = pretty(range(meta$cvdepy, na.rm = T)),
         cex.axis = 0.75,
    ...
    for(o in 1:length(v.outcomes)){ #loop runs over each outcome 'o' and each drug class 't'
      ...
      for(t in 1:length(v.type)){
       ...
       # Model (random effects) for outcome o class t
        ...
        # Primary outcomes
        for(i in 1:7){
          cc.s1[i,3] <- round(s.g[[i]]$beta[2],3)
          cc.s1[i,4] <- round(s.g[[i]]$ci.lb[2],3)
          cc.s1[i,5] <- round(s.g[[i]]$ci.ub[2],3)
          cc.s1[i,6] <- round(s.g[[i]]$pval[2],3)
          cc.s1[i+7,3] <- round(s.s[[i]]$beta[2],3)
          cc.s1[i+7,4] <- round(s.s[[i]]$ci.lb[2],3)
          cc.s1[i+7,5] <- round(s.s[[i]]$ci.ub[2],3)
          cc.s1[i+7,6] <- round(s.s[[i]]$pval[2],3)
        }
        cc.s1 <- cc.s1[ccorder,]
        cc.s1
        # Export into text tables
      ...
      for(i in 1:4){
        ...
        if(i  >2){
          title(xlab = "Cardiovascular Mortality Rate (/100py) in Control Group", cex.lab = 0.8, line = 2.5)
        } else {
          title(xlab = " Cardiovascular Mortality Rate (/100py) in Control Group ", cex.lab = 0.8, line = 2.5)
        }
        # Axes
        axis(1, 
             at = pretty(range(l.do.sens[[i]]$cvdepy, na.rm = T)),
             cex.axis = 0.75,
             col.axis = "gray40", 
             col.ticks = "gray40", 
             col = "gray40", 
...
# S.2  Sensitivity Analysis CVD: Remove ELIXA and SOLOIST --------
  ...
  # Primary outcomes
  for(i in 1:7){
    cc.s2[i,3] <- round(s.g[[i]]$beta[2],3)
    cc.s2[i,4] <- round(s.g[[i]]$ci.lb[2],3)
    cc.s2[i,5] <- round(s.g[[i]]$ci.ub[2],3)
    cc.s2[i,6] <- round(s.g[[i]]$pval[2],3)
    cc.s2[i+7,3] <- round(s.s[[i]]$beta[2],3)
    cc.s2[i+7,4] <- round(s.s[[i]]$ci.lb[2],3)
    cc.s2[i+7,5] <- round(s.s[[i]]$ci.ub[2],3)
    cc.s2[i+7,6] <- round(s.s[[i]]$pval[2],3)
  }
  cc.s2 <- cc.s2[ccorder,]
  cc.s2
  # Export into text tables
...
# S3 =====
    ...
    # Primary outcome
    for(i in 1){
      cc.s3[i,3] <- round(s.g[[i]]$beta[2],3)
      cc.s3[i,4] <- round(s.g[[i]]$ci.lb[2],3)
      cc.s3[i,5] <- round(s.g[[i]]$ci.ub[2],3)
      cc.s3[i,6] <- round(s.g[[i]]$pval[2],3)
      cc.s3[i+1,3] <- round(s.s[[i]]$beta[2],3)
      cc.s3[i+1,4] <- round(s.s[[i]]$ci.lb[2],3)
      cc.s3[i+1,5] <- round(s.s[[i]]$ci.ub[2],3)
      cc.s3[i+1,6] <- round(s.s[[i]]$pval[2],3)
    }
...
# Primary outcomes
for(i in 1:7){
  cc.s4[i,3] <- round(s.g[[i]]$beta[2],3)
  cc.s4[i,4] <- round(s.g[[i]]$ci.lb[2],3)
  cc.s4[i,5] <- round(s.g[[i]]$ci.ub[2],3)
  cc.s4[i,6] <- round(s.g[[i]]$pval[2],3)
  cc.s4[i+7,3] <- round(s.s[[i]]$beta[2],3)
  cc.s4[i+7,4] <- round(s.s[[i]]$ci.lb[2],3)
  cc.s4[i+7,5] <- round(s.s[[i]]$ci.ub[2],3)
  cc.s4[i+7,6] <- round(s.s[[i]]$pval[2],3)
}
cc.s4 <- cc.s4[ccorder,]
cc.s4
# Export into text tables
...
# Primary outcomes
for(i in 1:7){
  cc.s5[i,3] <- round(s.g[[i]]$beta[2],3)
  cc.s5[i,4] <- round(s.g[[i]]$ci.lb[2],3)
  cc.s5[i,5] <- round(s.g[[i]]$ci.ub[2],3)
  cc.s5[i,6] <- round(s.g[[i]]$pval[2],3)
  cc.s5[i+7,3] <- round(s.s[[i]]$beta[2],3)
  cc.s5[i+7,4] <- round(s.s[[i]]$ci.lb[2],3)
  cc.s5[i+7,5] <- round(s.s[[i]]$ci.ub[2],3)
  cc.s5[i+7,6] <- round(s.s[[i]]$pval[2],3)
}
cc.s5 <- cc.s5[ccorder,]
cc.s5
# Export into text tables
...
for(i in 1:4){
  ...
  plot(x = 0, y = 0, type ='n', 
       xlim = range(l.do.sens[[i]]$cvdepy, na.rm = T), 
       ylim = c(-3,14), 
       xlab= "", 
       ylab ="", 
       axes = F)
...
# Primary outcomes
for(i in 1){
  cc.s6[i,3] <- round(s.g[[i]]$beta[2],3)
  cc.s6[i,4] <- round(s.g[[i]]$ci.lb[2],3)
  cc.s6[i,5] <- round(s.g[[i]]$ci.ub[2],3)
  cc.s6[i,6] <- round(s.g[[i]]$pval[2],3)
  cc.s6[i+1,3] <- round(s.s[[i]]$beta[2],3)
  cc.s6[i+1,4] <- round(s.s[[i]]$ci.lb[2],3)
  cc.s6[i+1,5] <- round(s.s[[i]]$ci.ub[2],3)
  cc.s6[i+1,6] <- round(s.s[[i]]$pval[2],3)
}
...
for(i in 1:1){
  ...
  if(i  >2){
    title(xlab = "Cardiovascular Mortality Rate (/100py) in Control Group", cex.lab = 0.8, line = 1.6)
  } else {
    title(xlab = " Cardiovascular Mortality Rate (/100py) in Control Group ", cex.lab = 0.8, line = 1.6)
  }
  # Axes
