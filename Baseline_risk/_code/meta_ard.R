...
df.ard$outcomelab <- ifelse(df.ard$outcome == "allcauseMort", "All-cause mortality",
                          df.ard$outcomelab)
df.ard$outcomelab <- ifelse(df.ard$outcome == "CVMort", "Cardiovascular mortality",
                          df.ard$outcomelab)
df.ard$outcomelab <- ifelse(df.ard$outcome == "HospHeartFailure", "Hospitalization for heart failure",
                          df.ard$outcomelab)
df.ard$outcomelab <- ifelse(df.ard$outcome == "stroke", "Stroke",
                          df.ard$outcomelab)
df.ard$outcomelab <- ifelse(df.ard$outcome == "MI", "Myocardial infarction",
                          df.ard$outcomelab)
df.ard$outcomelab <- ifelse(df.ard$outcome == "sustGFRdecl", "Composite renal outcome",
                          df.ard$outcomelab)
...
df.ard <- subset(df.ard, subset = !(df.ard$trialname == "LEADER/SUSTAIN-6"))
df.ard$ardp <- df.ard$ard
df.ard$ard <- -df.ard$ardp
...
for(i in 1:length(outcomes)){
  ...
  forest.meta(model.g, 
         ...
         col.square  = "plum4",
         leftcols = c("studlab"),
         leftlabs = paste0(unique(d.o$outcomelab), "\nGLP-1RAs"),
         label = T,
...
p * theme_minimal()
p
ggsave("~/Documents/_meta/_rev3/Figure2.tiff",plot = p, dpi = 1000, width =  12, height =  16)
...
df.ard <- subset(df.ard, subset = !(trialname == "CREDENCE" & outcome == "MI"))
df.ard <- subset(df.ard, subset = !(trialname == "CREDENCE" & outcome == "stroke"))
df.ard <- subset(df.ard, subset = !(trialname == "CANVAS program" & outcome == "allcauseMort"))
for(i in 1:length(outcomes2)){
  # I: Subset data per outcome/drugclass
  d.o <- subset(df.ard, subset = (df.ard$outcomelab == outcomes2[i]))
  ...
  # III. Forest plot
  # III.a GLP-1Ra
  tiff(paste0("~/Documents/_meta/_forest/meta_ard_glp1_", outcomes2[i],".tif"), 
  ...
  dev.off()
  # III.b SGLT2i  
  tiff(paste0("~/Documents/_meta/_forest/meta_ard_sglt2_", outcomes2[i],".tif"), 
      width = 6, height = 4, units = 'in', res = 1000) 
  forest(l.ms2[[i]], 
         #atransf  = log,
         sortvar = d.s$ordertrial,
  ...
  lnames.g2[[i]] <- paste0("~/Documents/_meta/_forest/meta_ard_glp1_", outcomes2[i],".tif")
  lnames.s2[[i]] <- paste0("~/Documents/_meta/_forest/meta_ard_sglt2_", outcomes2[i],".tif")
...
ggsave("/Users/jose/Documents/_meta/_rev3/metaardall.png",plot = p2.1, dpi = 300, width =  12, height =  16)
ggsave("/Users/jose/Documents/_meta/_rev3/metaardmi.png",plot = p2.2, dpi = 300, width =  12, height =  16)
ggsave("/Users/jose/Documents/_meta/_rev3/metaardstr.png",plot = p2.3, dpi = 300, width =  12, height =  16)
